...
### 核心修复
- **新增 `docs/HINT_SYSTEM_RULES.md`**：提示系统不可绕过契约文档
- **新增 `validate:hints`**：源码扫描 + 题库泄题检查 + G1/G2 高阶术语检查
- **324 题全部通过提示安全检查**（0 errors）
- **修复 3 个源码文件中的硬编码"小提示"**（已移至 HintSystem 统一渲染）
- **修复 4 个题目的 light hint 泄题**（g1t_23, g2o_15, g5o_21, oi_28）
- **validate:hints 加入 package.json scripts**
